...
Given two parameters, `count` and `var`

- initialize Array, `results`, with one element, `var`
- (`count` - 1) number of times do the following:
  - add the sum of `var` + the last element of `results` to `results`
- return `results`
...
def sequence(count, var)
  if count > 0
    results = [var]
    (count - 1).times { results << (var + results.last) }
    return results
  else
    return []
  end
